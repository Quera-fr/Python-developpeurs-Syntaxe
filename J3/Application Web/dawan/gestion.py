...
def get_article(text_input:str='Python') -> None:
    ...
    database = DataBase('article')
    table_name = text_input.replace(' ', '_')
    ...
    for article in articles:
        ...
        try: 
            ...
            url = 'None'
        try:
            titre = article.find_element(By.TAG_NAME, 'h3').text
        except:
            titre = 'None'
        ...
        with col1:
            st.subheader(titre)
            st.link_button("Accédez à l'article", url)
        with col2:
            st.image(image_url)

        database.insert_to_table(table_name,
                                 titre=titre,
                                 image_url=image_url,
                                 url=url)
